...
class NumberStatus extends Component {
  ...
  queryNumber = async () => {
    ...
    console.log(message);
    let res = await this.props.callApi2(RestApi.sendRequestNew, {'mod': mod, 'message': message.toUpperCase(), 'timeout': timeout});
  ...
  updateQuery = async () => {
    ...
    console.log(message);
    let res = await this.props.callApi2(RestApi.sendRequestNew, {'mod': 'NSC', 'message': message.toUpperCase(), 'timeout': timeout});
...
export default connect((state) => ({somos: state.auth.profile.somos, token: state.auth.token, }))(withLoadingAndNotification(NumberStatus));
